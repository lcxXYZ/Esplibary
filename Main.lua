...
local function createEsp(player)
    local esp = {
        ...
        -- Removed redundant 'tracer' definition here
        tracer = create("Line", {
...
local function updateEsp()
    for player, esp in pairs(cache) do
        -- Use the global settings table for all checks and properties
        ...
        if character and (not ESP_SETTINGS.Teamcheck or (team and team ~= localPlayer.Team)) then
            ...
            if rootPart and head and humanoid and shouldShow then
                ...
                if onScreen then
                    ...
                    -- Calculate box size based on screen position
                    ...
                    if ESP_SETTINGS.ShowBox and ESP_SETTINGS.Enabled then
                        if ESP_SETTINGS.BoxType == "2D" then
                            ...
                            esp.boxLines = {} -- Clear box lines for 2D mode
                        elseif ESP_SETTINGS.BoxType == "Corner Box Esp" then
                            ...
                            for _, line in ipairs(boxLines) do
                                line.Color = ESP_SETTINGS.BoxColor -- Ensure all lines use the BoxColor for the main line
                                line.Visible = true
                            end
                            -- Ensure outline lines use the outline color (index 9-16)
                            for i = 9, 16 do
                                boxLines[i].Color = ESP_SETTINGS.BoxOutlineColor
                            end
...
-- We no longer return the table, as it's already available globally via getgenv().
